...
Données : MPXV clade Ib (Illumina paired-end, capture ciblée — Viral Surveillance Panel)
...
- `--length_required 30` — jette les lectures < 30 bases après rognage (seuil bas car les inserts de ce jeu sont courts).
- `--json` / `--html` — rapports (le JSON est lu par MultiQC).

> **Détection auto des adaptateurs** — `--detect_adapter_for_pe` laisse fastp identifier l'adaptateur tout seul, ce qui est robuste quand on ne connaît pas le kit exact.

> **Adapter le seuil de longueur à l'insert** — fastp rapporte l'« insert size peak » (taille du fragment ADN réel). Si les inserts sont courts, beaucoup de lectures deviennent courtes après rognage. Un seuil `--length_required` trop haut les jetterait et ferait perdre du signal viral. On abaisse alors le seuil (ici 30) pour conserver ces lectures. À vérifier dans le rapport fastp.
...
- **fastp fait tout** : adaptateurs + qualité en une commande (pas d'étape d'amorces — la capture ciblée n'ajoute pas d'amorces PCR dans les lectures, contrairement à l'amplicon).
- **Détection auto des adaptateurs** : robuste quand le kit est inconnu.
- **Lectures non virales résiduelles** : la capture enrichit le virus mais ne l'isole pas totalement ; le tri final se fait au mapping.
- **Ne pas sur-nettoyer** : garder de la marge pour ne pas perdre les lectures virales.